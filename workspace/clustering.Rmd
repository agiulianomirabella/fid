...
plot_clustering <- function(dataframe, centers, save_path) {
  for (column1 in names(dataframe)) {
    for (column2 in names(dataframe)) {
      if (column2 != column1) {
        km_df <- subset(dataframe, select = c(column1, column2))
        km <- kmeans(km_df, center=centers, nstart=500)
        plot_filename <- paste(SAVE_PATH, save_path, column1, "_", column2, "_", "k_", centers, ".png",sep = "")
        png(filename=plot_filename)
        plot(km_df, col=km$cluster, main=paste(centers, " clusters"))
        dev.off()
      }
    }
  }
...
```{r}
centers = 3
# Plot de todas las columnas no binarias:
kmeans_df <- subset(scaled_df, select = setdiff(names(scaled_df), binary_columns))
plot_clustering(kmeans_df, centers, "simple/")
```
...
centers <- 5
plot_clustering(pca, centers, "pca/")
```


A continuación, vamos a estudiar el efecto que tiene el número de clusters (k) sobre la bondad del clustering. Como la técnica forma parte del aprendizaje no supervisado, no puede utilizarse la columna a predecir (death) para medir la bondad del clustering, sino que se utiliza una medida llamada silhouette, que estima la distancia intracluster y la intercluster. El clustering será mejor cuanto más baja sea la suma de estas dos distancias, es decir, cuanto más baja sea la silhouette. Primero definimos una función genérica que calcule la evolución de la silhouette según k: 
...
plot_silhouette_by_centers <- function(dataframe, save_name){
  ...
  png(filename=plot_filename)
  plot(k, type='b', mean_sil, main= "Silhouette evolution by number of centers", xlab='Number of centers', ylab='Silhouette', frame=FALSE)
  dev.off()
}
```
...
plot_h_clustering <- function(dataframe, save_name) {
  for (m in c("complete", "average", "single")) {
    ...
    plot(hclust.complete, main = paste("Hclustering del dataframe ", save_name, " con distancia: '", m, "'", sep=""))
    dev.off()
  }
}
